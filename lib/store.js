...
export class MemoryStore {
  ...
  _invalidate(dir, file) {
    // A refreshed layer may have exactly the same signature after an equal-size write.
    this._merged = null
    const prev = this._cache.get(dir)
    if (!prev) return
   ...
   * 被引用但还没写的名字，连同是谁在引用。
   *
   * 这不是错误清单，是**待写清单** —— 鼓励模型链到还不存在的记忆，正是为了让
   * "这件事以后值得记"能留下痕迹；这个方法把那些痕迹收集成可以逐条消化的东西。
   */
  unwrittenLinks() {
    ...
    for (const entry of entries) {
      for (const link of entry.links) {
        ...
        if (!pending.has(link)) pending.set(link, [])
        pending.get(link).push(entry.name)
      }
    }
  ...
  /** 正文原文。超大条目不进缓存，那时按需读盘。 */
  bodyOf(entry) {
    if (!entry.oversized) return entry.body
    try {
      return parseMemory(readFileSync(entry.path, 'utf8'))?.body ?? ''
    } catch {
      return ''
    }
  }
  ...
  write({ name, description, type, content, layer }) {
    ...
    const existing = this._scan(layer).find((entry) => entry.file === basename)
    const previousCreated = existing ? new Date(existing.created).toISOString() : undefined
  ...
  /**
   * 就地改写已存在的一条：只换正文和元数据，`created` 保留。
   *
   * frontmatter 由这里重新渲染，调用方给的只是正文字符串 —— 所以局部编辑无论
   * 匹配到什么，都碰不到 `name` / `created`，也塞不进第二个文档头。
   */
  update({ name, body, description, type }) {
